...
def _format_mcp_connection_error(
    name: str, command: str = "", args: Optional[List[str]] = None, error: Exception = None
) -> str:
    ...
    async def call_tool(self, qualified_name: str, arguments: Dict) -> Dict:
        ...
        try:
            ...
            if self.is_builtin(server_id):
                ...
                if reconnected:
                    ...
                    if session:
                        try:
                            ...
                            logger.exception(
                                "MCP tool call failed after reconnect: %s tool_name=%s args=%r exc_type=%s exc_repr=%r",
                                qualified_name,
                                tool_name,
                                arguments,
                                type(e2).__name__,
                            ...
                            return {
                                "error": f"{type(e2).__name__}: {repr(e2)}",
                                "exit_code": 1,
                            }
                    else:
                ...
                logger.exception(
                    "MCP tool call failed: %s tool_name=%s args=%r exc_type=%s exc_repr=%r",
                    qualified_name,
                    tool_name,
                    arguments,
                    type(e).__name__,
